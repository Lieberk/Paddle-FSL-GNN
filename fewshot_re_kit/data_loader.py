    ...
    def __getitem__(self, index):
        ...
        for i, class_name in enumerate(target_classes):
            ...
            for j in indices:
                ...
                word = paddle.to_tensor(word).cast('int64')
                pos1 = paddle.to_tensor(pos1).cast('int64')
                pos2 = paddle.to_tensor(pos2).cast('int64')
                mask = paddle.to_tensor(mask).cast('int64')
                if count < self.K:
...
def collate_fn(data):
    ...
    for k in batch_support:
        batch_support[k] = paddle.stack(batch_support[k], 0)
    for k in batch_query:
        batch_query[k] = paddle.stack(batch_query[k], 0)
...
def get_loader(name, encoder, N, K, Q, batch_size,
               num_workers=8, collate_fn=collate_fn, na_rate=0, root='./pretrain/fewrel_wiki'):
...
def collate_fn_pair(data):
    ...
    for i in range(len(fusion_sets)):
        ...
        batch_label += query_labels[i]
    for k in batch_set:
        batch_set[k] = paddle.stack(batch_set[k], 0)
    batch_label = paddle.to_tensor(batch_label)
