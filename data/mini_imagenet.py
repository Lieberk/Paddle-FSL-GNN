...
class MiniImagenet(data.Dataset):
    def __init__(self, root, dataset='miniImagenet'):
        super().__init__()
        self.root = root
    ...
    def _init_folders_(self):
        ...
        if not os.path.exists(os.path.join(self.root, 'miniImagenet')):
            os.makedirs(os.path.join(self.root, 'miniImagenet'))
            decompress = True
    ...
    def check_decompress(self):
        return os.listdir('%s/miniImagenet' % self.root) == []

    def _decompress_(self):
        print("\nDecompressing Images...")
        compressed_file = '%s/compressed/miniImagenet/images.zip' % self.root
        if os.path.isfile(compressed_file):
            os.system('unzip %s -d %s/miniImagenet/' % (compressed_file, self.root))
        else:
    ...
    def get_image_paths(self, file):
        ...
        with open(file, 'r') as f:
            ...
            for line in f:
                ...
                path = self.root + '/miniImagenet/images/'+name
    ...
    def _preprocess_(self):
        ...
        (class_names_train, images_path_train) = self.get_image_paths('%s/miniImagenet/train.csv' % self.root)
        (class_names_test, images_path_test) = self.get_image_paths('%s/miniImagenet/test.csv' % self.root)
        (class_names_val, images_path_val) = self.get_image_paths('%s/miniImagenet/val.csv' % self.root)
