...
class Generator(data.Dataset):
    def __init__(self, root, args, partition='train', dataset='miniImagenet'):
        super().__init__()
        self.root = root
    ...
    def get_task_batch(self, batch_size=5, n_way=20, num_shots=1, unlabeled_extra=0):
        ...
        batches_xi = [paddle.to_tensor(batch_xi) for batch_xi in batches_xi]
        labels_yi = [paddle.to_tensor(label_yi) for label_yi in labels_yi]
        oracles_yi = [paddle.to_tensor(oracle_yi) for oracle_yi in oracles_yi]

        labels_x_scalar = np.argmax(labels_x, 1)

        return_arr = [paddle.to_tensor(batch_x), paddle.to_tensor(labels_x), paddle.to_tensor(labels_x_scalar),
                      paddle.to_tensor(labels_x_global), batches_xi, labels_yi, oracles_yi,
                      paddle.to_tensor(hidden_labels)]
